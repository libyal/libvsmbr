 ...
 */
int vsmbr_test_handle_get_narrow_source(
     const system_character_t *source,
     char *narrow_string,
     size_t narrow_string_size,
     libcerror_error_t **error )
{
	static char *function     = "vsmbr_test_handle_get_narrow_source";
 ...
 * Returns 1 if successful or -1 on error
 */
int vsmbr_test_handle_get_wide_source(
     const system_character_t *source,
     wchar_t *wide_string,
     ...
     const system_character_t *source )
{
	...
	/* Initialize test
	 */
	result = vsmbr_test_handle_get_narrow_source(
	          source,
	          narrow_source,
	          256,
	          &error );
     ...
     const system_character_t *source )
{
	...
	/* Initialize test
	 */
	result = vsmbr_test_handle_get_wide_source(
	          source,
	          wide_source,
	          256,
	          &error );
...
#endif
{
	...
	if( result != 0 )
	{
		...
		/* TODO add test for libvsmbr_handle_open_file_io_handle */
...
on_error:
	...
	if( handle != NULL )
	{
		vsmbr_test_handle_close_source(
		 &handle,
		 NULL );
	}
	return( EXIT_FAILURE );
