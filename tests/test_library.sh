...
LIBRARY_TESTS="chs_address error io_handle notify partition partition_values section_values support volume_type";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./vsmbr_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libvsmbr" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
