...
dnl Function to detect if vsmbrtools dependencies are available
AC_DEFUN([AX_VSMBRTOOLS_CHECK_LOCAL],
...
AC_DEFUN([AX_LIBVSMBR_CHECK_DLL_SUPPORT],
  [AS_IF(
    ...
    [AS_CASE(
      ...
      AC_SUBST(
        [LIBVSMBR_DLL_EXPORT],
        ["-DLIBVSMBR_DLL_EXPORT"])

      AC_SUBST(
        [LIBVSMBR_DLL_IMPORT],
        ["-DLIBVSMBR_DLL_IMPORT"])
      ])
    ])
  ])
